/*
 * This code below comes from OpenSolaris outsourced code, and has
 * been extended to add -f flag that allow us to specify location
 * of the output file, exactly as the GNU version does.
 *										Michal Nowak
 */
...
//	(void) textdomain(TEXT_DOMAIN);
	...
	while ((c = getopt(argc, argv, "ac:f:")) != -1) {
		switch (c) {
			...
			default:
				fprintf(stderr,
						"Usage: Script [ -a ] [ -c cmd ] [ -f filename]\n");
				exit(1);
		}
	}
